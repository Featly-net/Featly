...
- **`db` commands run OFFLINE** — directly against the database through a public per-provider migration runner (`SqliteMigrationRunner`, `PostgresMigrationRunner`, `SqlServerMigrationRunner`) keeping each `FeatlyDbContext` internal ([ADR-0006](0006-ef-core-internal-dbcontext.md)). `--provider sqlite|postgres|sqlserver` (default `sqlite`) selects which; connection string from `--connection-string` / a per-provider environment variable (`FEATLY_SQLITE` / `FEATLY_POSTGRES` / `FEATLY_SQLSERVER`) / a default for SQLite only — Postgres and SQL Server have none and fail fast, mirroring `AddFeatlyPostgresStore()`/`AddFeatlySqlServerStore()` (issues #257, #274).
