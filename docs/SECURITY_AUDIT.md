...
- API keys are environment-scoped ([ADR-0009](adr/0009-api-keys-per-environment-scoped.md)); a leaked SDK key reads one environment and cannot mutate. The binding is **enforced at request time**: a persisted key carries its `EnvironmentId` as a principal claim, and the SDK endpoints (`/config`, `/stream`, `/events`) return `403` when the resolved environment differs from it — so a key scoped to `staging` cannot read `production` by changing `?env=` (regression test `SdkEnvironmentScopeTests`). The static appsettings bootstrap key is intentionally unbound (wildcard) for the single-environment quickstart. Because it is shared, unattributable, and non-rotatable, the server logs a startup **warning** once a real role-bound admin user exists, advising the operator to remove it and rely on per-user admin keys ([issue #209](https://github.com/Featly-net/Featly/issues/209)); it is not disabled automatically, since silently revoking a live credential could lock an operator out.
