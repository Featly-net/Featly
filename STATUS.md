...
The cut followed [docs/RELEASING.md](docs/RELEASING.md) end to end — first use of the written process. Since then the `[Unreleased]` changelog holds the dashboard modernisation under ADR-0036 and a server validation fix:

- **Dashboard ES baseline is decided and the syntax conversion is done** (issue #229, ADR-0036): ES2020 on evergreen browsers, still served verbatim with no build step. The rule-family slices landed one PR each with lint + Playwright smoke + rendered-DOM diff against `main` as proof: `indexOf`→`includes` / `Number.*` (#320), `dataset` (#321), optional chaining (#322), `replaceAll` (#325), `var`→`const`/`let` with `no-var`/`prefer-const` now lint errors (#327), plus a shared list-row navigation handler (#323). What remains of #229 is the split into native ES modules, which is also where the S2004 nesting findings get addressed.
- Admin write endpoints reject bodies missing required members with an RFC 7807 400 instead of an unhandled 500 (#324 → #326).
...
Also shipped in `v0.0.9-preview.1`: the M10 polish PR (ReadOnly env lock/unlock + full audit filters). Deferred M10 line items remain in `docs/DEFERRED.md`.
