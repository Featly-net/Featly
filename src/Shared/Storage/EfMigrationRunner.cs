...
/// public status record, then hands the open context to these methods, which
/// are otherwise byte-identical across providers. Compiled into each provider
/// assembly as a linked source file, same pattern as the <c>Ef*Store</c> bases.
/// </remarks>
internal static class EfMigrationRunner
{
    ...
    public static async Task MigrateAsync(DbContext db, CancellationToken ct)
    {
    ...
    public static async Task<EfMigrationStatus> GetStatusAsync(DbContext db, CancellationToken ct)
    {
        ArgumentNullException.ThrowIfNull(db);
    ...
    /// </summary>
    public static async Task RollbackAsync(DbContext db, string targetMigration, CancellationToken ct)
    {
        ArgumentNullException.ThrowIfNull(db);
    ...
    public static async Task<bool> DropAsync(DbContext db, CancellationToken ct)
    {
        ArgumentNullException.ThrowIfNull(db);
