...
internal sealed class MySqlSegmentStore(IDbContextFactory<FeatlyDbContext> contextFactory) : ISegmentStore
{
    ...
    public async Task UpsertAsync(Guid environmentId, Segment segment, string actor, CancellationToken ct)
    {
        ...
        {
            // Conditions is a plain JSON-column-backed scalar here
            // (JsonCollectionConversion), not an EF owned collection, so a
            // direct assignment is the whole update.
