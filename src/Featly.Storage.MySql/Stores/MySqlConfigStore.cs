...
internal sealed class MySqlConfigStore(IDbContextFactory<FeatlyDbContext> contextFactory) : IConfigStore
{
    ...
    public async Task UpsertAsync(Guid environmentId, Config config, string actor, CancellationToken ct)
    {
        ...
        {
            // Tags/Rules are plain JSON-column-backed scalars here
            // (JsonCollectionConversion), not EF owned collections, so a
