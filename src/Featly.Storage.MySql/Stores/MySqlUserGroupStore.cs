...
internal sealed class MySqlUserGroupStore(IDbContextFactory<FeatlyDbContext> contextFactory) : IUserGroupStore
{
    ...
    public async Task<IReadOnlyList<UserGroup>> ListForMemberAsync(Guid userId, CancellationToken ct)
    {
        ...
        // MemberUserIds is a plain JSON-column-backed scalar here
        // (JsonCollectionConversion), not an EF primitive collection — Pomelo
        // doesn't translate Contains() against it server-side (see ADR-0033),
        // unlike the other providers' native array/JSON-path support. Group
        // counts are expected to stay small, so filtering the full list in
        // memory is the pragmatic choice over hand-rolled raw SQL.
        var all = await db.UserGroups.AsNoTracking()
            .ToListAsync(ct)
            .ConfigureAwait(false);
        return [.. all.Where(g => g.MemberUserIds.Contains(userId))];
    }
