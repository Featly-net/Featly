...
internal sealed class SqliteExperimentStore(IDbContextFactory<FeatlyDbContext> contextFactory) : IExperimentStore
{
    ...
    {
        await using var db = await contextFactory.CreateDbContextAsync(ct).ConfigureAwait(false);
        // Active == started and not stopped. StartedAt/StoppedAt persist as nullable ticks.
        return await db.Experiments.AsNoTracking()
