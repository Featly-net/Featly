...
internal sealed class SqlServerUserGroupStore(IDbContextFactory<FeatlyDbContext> contextFactory) : IUserGroupStore
{
    ...
    {
        await using var db = await contextFactory.CreateDbContextAsync(ct).ConfigureAwait(false);
        // EF translates Contains over a primitive collection to a JSON query.
        return await db.UserGroups.AsNoTracking()
