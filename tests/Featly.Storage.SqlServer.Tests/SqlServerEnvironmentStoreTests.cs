...
public class SqlServerEnvironmentStoreTests
{
    [Fact]
    ...
    [Fact]
    public async Task BumpConfigVersionAsync_increments_and_is_idempotent_for_missing_id()
    {
        await using var host = await SqlServerTestHost.CreateAsync(TestContext.Current.CancellationToken);
        var ct = TestContext.Current.CancellationToken;
        var store = host.EnvironmentStore;

        var env = new Environment { Id = Guid.NewGuid(), ProjectId = Guid.NewGuid(), Key = "bump-me", Name = "Bump", CreatedAt = DateTimeOffset.UtcNow };
        await store.CreateAsync(env, ct);

        await store.BumpConfigVersionAsync(env.Id, ct);
        await store.BumpConfigVersionAsync(env.Id, ct);

        var updated = await store.GetByIdAsync(env.Id, ct);
        updated!.ConfigVersion.Should().Be(2);
