...
/// <summary>
/// Covers the consumer-facing entry point (issue #274, mirroring #179):
/// <c>AddFeatlySqlServerStore()</c> must resolve a fully wired
/// <see cref="StorageFacade"/> that actually talks to SQL Server — the piece
/// that turns 19 working sub-stores into a usable provider.
/// </summary>
[Trait("Category", "RequiresSqlServer")]
public class SqlServerStoreRegistrationTests
{
    [Fact]
    public async Task AddFeatlySqlServerStore_resolves_a_facade_that_round_trips()
    {
        ...
        services.AddFeatlySqlServerStore(o => o.ConnectionString = host.ConnectionString);
    ...
    [Fact]
    public void AddFeatlySqlServerStore_without_a_connection_string_fails_fast()
    {
        // There is no sensible default server, so this must fail at startup with
        // a clear message rather than at the first query with a SqlException.
        var services = new ServiceCollection();
        services.AddLogging();
        // A real host supplies IConfiguration; BindConfiguration needs it.
        services.AddSingleton<IConfiguration>(new ConfigurationBuilder().Build());
        services.AddFeatlySqlServerStore();
        using var provider = services.BuildServiceProvider();

        var resolve = () => provider.GetRequiredService<IOptions<SqlServerFeatlyStoreOptions>>().Value;
