...
/// <summary>
/// Round-trips for the DB-overridable settings store (ARCHITECTURE.md §15): a
/// typed-singleton row keyed by aggregate key, with a JSON payload and audit
/// stamps. Upsert is replace-in-place by key.
/// </summary>
public class SqliteSystemSettingsStoreTests
{
    ...
    public async Task Upsert_then_get_round_trips_the_payload()
    {
        ...
        await host.Store.Settings.UpsertAsync(setting, ct);

        var loaded = await host.Store.Settings.GetAsync("webhook", ct);
    ...
    public async Task Upsert_replaces_in_place_by_key()
    {
        ...
        await host.Store.Settings.UpsertAsync(new SystemSetting
        {
            Key = "webhook",
        ...
        (await host.Store.Settings.ListAsync(ct)).Should().ContainSingle();
        var loaded = await host.Store.Settings.GetAsync("webhook", ct);
    ...
    {
        await using var host = await SqliteTestHost.CreateAsync(TestContext.Current.CancellationToken);
        (await host.Store.Settings.GetAsync("missing", TestContext.Current.CancellationToken)).Should().BeNull();
    }
    ...
    public async Task List_returns_every_singleton_ordered_by_key()
    {
        ...
        await host.Store.Settings.UpsertAsync(new SystemSetting { Key = "webhook", Payload = Json(new { a = 1 }), UpdatedAt = DateTimeOffset.UtcNow }, ct);
        await host.Store.Settings.UpsertAsync(new SystemSetting { Key = "authorization", Payload = Json(new { b = 2 }), UpdatedAt = DateTimeOffset.UtcNow }, ct);

        var all = await host.Store.Settings.ListAsync(ct);
