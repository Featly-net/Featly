...
/// <summary>
/// Round-trip coverage for <c>MySqlConfigStore</c>, mirroring
/// <c>SqlServerConfigStoreTests</c>/<c>PostgresConfigStoreTests</c>.
/// <c>Config.Rules</c> is a plain scalar serialized into a native
/// <c>json</c> column (<c>JsonCollectionConversion</c>), and
/// <c>DefaultValue</c> is a bare <c>JsonElement</c> scalar
/// (<c>JsonScalarConversion</c>) — this proves both hold across every
/// supported <see cref="ConfigType"/>.
/// </summary>
[Trait("Category", "RequiresMySql")]
public class MySqlConfigStoreTests
{
    [Fact]
    public async Task Upsert_persists_config_with_default_value_tags_and_rules()
    {
        ...
        var loaded = await store.GetAsync(envId, "checkout.timeout", ct);
    ...
    public async Task Upsert_overwrites_existing_config_and_keeps_id()
    {
        ...
        var originalId = (await store.GetAsync(envId, "checkout.timeout", ct))!.Id;
    ...
    public async Task ListAsync_filters_archived_and_scopes_per_environment()
    {
        ...
        await store.UpsertAsync(envA, NewIntConfig(envA, "alpha"), "t", ct);
        await store.UpsertAsync(envA, NewIntConfig(envA, "beta"), "t", ct);
        await store.UpsertAsync(envB, NewIntConfig(envB, "gamma"), "t", ct);
    ...
    public async Task Round_trips_every_supported_config_type(ConfigType type, string rawJsonValue)
    {
        ...
        await store.UpsertAsync(envId, config, "test", ct);

        var loaded = await store.GetAsync(envId, "any", ct);

        loaded.Should().NotBeNull();
        loaded!.Type.Should().Be(type);

        // Structural, not byte-for-byte: MySQL's native json column type
        // stores an optimized internal representation and always returns a
        // canonical text form on read (e.g. inserting a space after `:`/`,`
        // in objects) — a real, documented MySQL behavior, not a mapping
        // bug. Every other provider here maps this scalar as plain text, so
        // GetRawText() equality holds there but not against MySQL's native
        // type.
        JsonElement.DeepEquals(loaded.DefaultValue, JsonDocument.Parse(rawJsonValue).RootElement)
            .Should().BeTrue();
    }
