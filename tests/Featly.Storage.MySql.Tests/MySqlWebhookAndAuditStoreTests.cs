...
/// <c>SqlServerWebhookAndAuditStoreTests</c>/<c>PostgresWebhookAndAuditStoreTests</c>:
/// <c>MySqlWebhookStore</c> (with the subscribed event-types collection),
/// the persisted <c>MySqlWebhookDeliveryStore</c> queue (due claim + result
/// update), and the append-only <c>MySqlAuditStore</c> log (filtered query
/// with a JsonElement payload).
/// </summary>
[Trait("Category", "RequiresMySql")]
public class MySqlWebhookAndAuditStoreTests
{
    [Fact]
