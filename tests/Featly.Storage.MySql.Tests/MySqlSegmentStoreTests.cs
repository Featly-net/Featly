...
/// <summary>
/// Round-trip coverage for <c>MySqlSegmentStore</c>, mirroring
/// <c>SqlServerSegmentStoreTests</c>/<c>PostgresSegmentStoreTests</c>.
/// <c>Segment.Conditions</c> is a plain scalar serialized into a native
/// <c>json</c> column via <c>JsonCollectionConversion</c> (not an EF owned
/// entity — Pomelo doesn't yet implement <c>OwnsMany().ToJson()</c>, see
/// ADR-0033) — this proves the mapping holds for nested condition operators
/// (Equals, In) and array values despite the different mechanism.
/// </summary>
[Trait("Category", "RequiresMySql")]
public class MySqlSegmentStoreTests
{
