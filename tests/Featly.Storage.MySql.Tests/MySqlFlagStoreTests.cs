...
/// <summary>
/// Round-trip coverage for <c>MySqlFlagStore</c>, mirroring
/// <c>SqlServerFlagStoreTests</c>/<c>PostgresFlagStoreTests</c>. The
/// rules/variants round-trip is the highest-risk part of the MySQL mapping:
/// <c>Flag.Variants</c>/<c>Rules</c>/<c>Tags</c>/<c>Prerequisites</c> are
/// plain scalar properties serialized into native <c>json</c> columns via
/// <c>JsonCollectionConversion</c> (not EF owned entities — Pomelo doesn't yet
/// implement <c>OwnsMany().ToJson()</c>, see ADR-0033) — this proves the
/// mapping holds for nested conditions, weighted splits, and a boolean
/// JsonElement value despite the different mechanism.
/// </summary>
[Trait("Category", "RequiresMySql")]
public class MySqlFlagStoreTests
{
    [Fact]
