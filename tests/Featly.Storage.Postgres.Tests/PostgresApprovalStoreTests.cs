...
/// <summary>
/// Round-trips for <c>PostgresPendingChangeStore</c> (with owned
/// approvals + comments and JsonElement proposed/current state) and
/// <c>PostgresApprovalPolicyStore</c> (with owned approver rules, one per
/// environment), mirroring <c>SqliteApprovalStoreTests</c>.
/// </summary>
[Trait("Category", "RequiresPostgres")]
public class PostgresApprovalStoreTests
{
    [Fact]
    public async Task PendingChange_round_trips_state_approvals_and_comments()
    {
        ...
        var loaded = await store.GetByIdAsync(changeId, ct);
    ...
    {
        // Same regression coverage as the SQLite provider's own test (a prior
        // version of PostgresPendingChangeStore.UpdateAsync copied fields one
        // by one and could have missed one the same way SQLite's did).
        await using var host = await PostgresTestHost.CreateAsync(TestContext.Current.CancellationToken);
        var ct = TestContext.Current.CancellationToken;
        var store = host.PendingChangeStore;
        ...
        var reloaded = await store.GetByIdAsync(change.Id, ct);
    ...
    public async Task ListByStatus_and_ListOpenForEntity_filter_correctly()
    {
        ...
        await store.CreateAsync(appliedOther, ct);

        (await store.ListByStatusAsync(ChangeStatus.Pending, ct)).Should().ContainSingle();
        // Open = Pending or Approved, for entity Flag/a in this env -> 2.
        (await store.ListOpenForEntityAsync("Flag", "a", env, ct)).Should().HaveCount(2);
        (await store.ListOpenForEntityAsync("Flag", "b", env, ct)).Should().BeEmpty(); // applied isn't open
    }
    ...
    public async Task ApprovalPolicy_round_trips_rules_and_is_one_per_environment()
    {
        ...
        await store.UpsertAsync(policy, ct);
        var reloaded = await store.GetByEnvironmentAsync(envId, ct);
