...
/// <c>MySqlWebhookAndAuditStoreTests</c>: <c>MongoWebhookStore</c> (with the
/// subscribed event-types collection), the persisted
/// <c>MongoWebhookDeliveryStore</c> queue (due claim + result update), and
/// the append-only <c>MongoAuditStore</c> log (filtered query with a
/// JsonElement payload and the tamper-evident hash chain).
/// </summary>
[Trait("Category", "RequiresMongoDB")]
public class MongoWebhookAndAuditStoreTests
{
    [Fact]
    ...
    [Fact]
    public async Task RecordCircuitState_does_not_get_reset_by_a_plain_upsert()
    {
        await using var host = await MongoTestHost.CreateAsync(TestContext.Current.CancellationToken);
        var ct = TestContext.Current.CancellationToken;
        var store = host.WebhookStore;
        ...
        await store.RecordCircuitStateAsync(endpoint.Id, consecutiveFailures: 3, openUntil, ct);

        // An admin edit (plain Upsert) must not clear a tripped circuit — it's
        // worker-managed.
        endpoint.Name = "Slack relay (renamed)";
        await store.UpsertAsync(endpoint, ct);

        var loaded = await store.GetByIdAsync(endpoint.Id, ct);
        loaded!.Name.Should().Be("Slack relay (renamed)");
        loaded.ConsecutiveFailures.Should().Be(3);
        // BSON's native Date type is millisecond precision, unlike .NET's
        // 100ns-tick DateTimeOffset — an exact Be() would flake on the
        // sub-millisecond remainder.
        loaded.CircuitOpenUntil.Should().BeCloseTo(openUntil, TimeSpan.FromMilliseconds(1));
    }
    ...
    [Fact]
    public async Task TryClaimDue_wins_exactly_once_for_concurrent_workers()
    {
        await using var host = await MongoTestHost.CreateAsync(TestContext.Current.CancellationToken);
        var ct = TestContext.Current.CancellationToken;
        var store = host.WebhookDeliveryStore;
        ...
        var delivery = NewDelivery(Guid.NewGuid(), now.AddSeconds(-5));
        await store.EnqueueAsync([delivery], ct);

        var lease = now.AddMinutes(1);
        var first = await store.TryClaimDueAsync(delivery.Id, now, lease, ct);
        var second = await store.TryClaimDueAsync(delivery.Id, now, lease, ct);

        first.Should().BeTrue();
        second.Should().BeFalse();
        (await store.GetByIdAsync(delivery.Id, ct))!.NextAttemptAt.Should().BeCloseTo(lease, TimeSpan.FromMilliseconds(1));
    }
    ...
    [Fact]
    public async Task Audit_hash_chain_links_sequential_entries_and_verifies()
    {
        await using var host = await MongoTestHost.CreateAsync(TestContext.Current.CancellationToken);
        var ct = TestContext.Current.CancellationToken;
        var store = host.AuditStore;

        var first = new AuditEntry { Id = Guid.NewGuid(), At = DateTimeOffset.UtcNow, Action = "flag.updated", EntityType = "Flag" };
        var second = new AuditEntry { Id = Guid.NewGuid(), At = DateTimeOffset.UtcNow, Action = "flag.updated", EntityType = "Flag" };
        await store.AppendAsync(first, ct);
        await store.AppendAsync(second, ct);
        ...
        second.Hash.Should().NotBeNullOrEmpty();

        var verification = await store.VerifyChainAsync(ct);
