...
/// <summary>
/// Round-trips for <c>MongoSystemSettingsStore</c>. Unlike every other entity
/// in this provider, <c>SystemSetting.Key</c> maps directly to <c>_id</c>
/// (there is no separate row id) — upsert uses a native
/// <c>ReplaceOneAsync(..., IsUpsert = true)</c> rather than a
/// find-then-insert-or-replace round trip.
/// </summary>
[Trait("Category", "RequiresMongoDB")]
public class MongoSystemSettingsStoreTests
{
