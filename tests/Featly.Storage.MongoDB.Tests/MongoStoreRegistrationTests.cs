...
/// <summary>
/// Covers the consumer-facing entry point (issue #277, mirroring #276/#274/#179):
/// <c>AddFeatlyMongoStore()</c> must resolve a fully wired
/// <see cref="StorageFacade"/> that actually talks to MongoDB — the piece
/// that turns 19 working sub-stores into a usable provider.
/// </summary>
[Trait("Category", "RequiresMongoDB")]
public class MongoStoreRegistrationTests
{
    [Fact]
    public async Task AddFeatlyMongoStore_resolves_a_facade_that_round_trips()
    {
        ...
        services.AddFeatlyMongoStore(o => o.ConnectionString = host.ConnectionString);
    ...
    [Fact]
    public void AddFeatlyMongoStore_without_a_connection_string_fails_fast()
    {
        // There is no sensible default server, so this must fail at startup
        // with a clear message rather than at the first query with a driver
        // exception.
        var services = new ServiceCollection();
        services.AddLogging();
        // A real host supplies IConfiguration; BindConfiguration needs it.
        services.AddSingleton<IConfiguration>(new ConfigurationBuilder().Build());
        services.AddFeatlyMongoStore();
        using var provider = services.BuildServiceProvider();

        var resolve = () => provider.GetRequiredService<IOptions<MongoFeatlyStoreOptions>>().Value;
