...
// Centralized deployment pattern, MongoDB-backed: unlike Centralized.Sample
// (SQLite, single writable replica), this host is designed to run as several
// replicas sharing one MongoDB replica set — see docker-compose.yml, which
// starts two of these behind a load balancer. A change written through one
// replica is pushed to every replica's SSE clients via a MongoDB Change
// Stream (ADR-0034, issue #277), not just the replica that made it.
//
// Point a consumer at either replica (or the load balancer) by setting, in the
// consumer's config:
//   Featly:Sdk:ServerUrl = http://localhost:5089
// The WebApi.Sample is a ready-made consumer.

var builder = WebApplication.CreateBuilder(args);

// MongoDB store shared by every replica. AutoMigrate is off here — see
// docker-compose.yml's dedicated one-shot "migrate" service, which applies the
// migration steps once before any replica starts, per docs/DEPLOYMENT.md's
// "Turn AutoMigrate off when you run more than one replica" guidance.
builder.Services.AddFeatlyMongoStore();
...
{
    service = "Featly.Samples.CentralizedMongoDB",
    replica = replicaName,
    role = "standalone Featly server, one of several replicas sharing one MongoDB replica set",
    dashboard = "/featly",
    health = "/health/live",
    firstRun = "bootstrap the first admin: `featly bootstrap-admin --identifier you@example.com --server-url http://localhost:5089`",
    consumers = "point an SDK app (e.g. WebApi.Sample) at Featly:Sdk:ServerUrl=http://localhost:5089",
}));
